...
class JointTorqueActionTerm(ActionTermBase):
    ...
    def __init__(self, cfg: ActionTermCfg, env: Any):
        ...
        self.wbc = [
            self._create_wbc_engine(resolved_extension_dir, resolved_params, self._wbc_module)
    ...
    def _compute_torques(self, actions: torch.Tensor) -> torch.Tensor:
        ...
        torques = torch.zeros(num_envs, self._torque_dim, device=actions.device, dtype=self.torques.dtype)
        ...
        for env_idx in range(num_envs):
            ...
            if np.linalg.norm(right_foot_grf[:3]) < force_threshold:
                right_foot_grf = 0 * right_foot_grf
                right_in_contact = False
            
            left_in_contact = True
            if np.linalg.norm(left_foot_grf[:3]) < force_threshold:
                left_foot_grf = 0 * left_foot_grf
            ...
            # # debug
            # print("right contact basis: ", right_contact_basis)
            # print("left contact basis: ", left_contact_basis)
            ...
            pelvis_ori = apply_delta_rotation(pelvis_rotation, action_dict["pelvis_ori"].detach().cpu().numpy())
            torso_ori = torso_pose[:3, :3]  # force to be the same right now
            ...
            right_foot_ori = apply_delta_rotation(right_foot_pose[:3, :3], action_dict["right_foot_ori"].detach().cpu().numpy())
            ...
            left_foot_ori = apply_delta_rotation(left_foot_pose[:3, :3], action_dict["left_foot_ori"].detach().cpu().numpy())
